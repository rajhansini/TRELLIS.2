"""
rung30_kl_both.py — rung27 + KL on BOTH attention maps, independently weighted
================================================================================
NEW FILE, copied from rung29_kl_cross_attention.py (which already carries three
fixes this file therefore inherits: the CheckpointError fix, w_kl in the config
hash, and both rung chains agreeing). rung27 and rung29 are untouched.

    L = L1 + 0.1*LPIPS + w_kl_cross * KL_cross + w_kl_self * KL_self

    KL_cross  softmax(q k^T) over the 1,029 IMAGE tokens   "where do I look in the photo"
    KL_self   softmax(q k^T) over the 1,999 VOXEL tokens   "who do I listen to in 3D"

BOTH q AND k MOVE IN BOTH MAPS, because --targets qkvo+sa adapts to_q, to_kv and
self_attn.to_qkv. Neither arm shares a query with the other.

TWO INDEPENDENT WEIGHTS, NOT ONE.  The two KLs are over different token counts
and different maps, so their magnitudes differ; a single weight on their sum
would let whichever is larger dominate and there would be no way to say which
site did the work. --kl-probe measures BOTH on the same run so each axis is
normalised by its own ratio.

MEMORY.  cross is [1999,16,1029] fp32 = 131 MB/block; self is [1999,16,1999] =
254 MB/block. Both at once across 30 blocks is ~11.5 GB for the lp_a tensors
alone, before the graph. --kl-voxels subsamples QUERIES (unbiased for the mean)
and the probe reports peak GPU so the grid is sized on a measurement.

rung29's header follows.
--------------------------------------------------------------------------------
rung29_kl_cross_attention.py — rung27 + a KL on the CROSS-ATTENTION distribution
================================================================================
NEW FILE, copied from rung27_selfattn_lora.py. rung27 is untouched and still runs.
--w-kl 0 reproduces rung27 exactly.

THE TERM
    L = L1 + 0.1*LPIPS + beta * KL( P_lora || P_frozen )
    P = softmax( q k^T / sqrt(d) )  over the 1,029 image tokens,
        meaned over voxel tokens and heads, per adapted block.

BOTH q AND k MOVE HERE, AND THAT IS THE DIFFERENCE FROM RUNG 26.
    rung26 adapted only to_kv, so q was FROZEN and shared by both arms:
        P_frozen = softmax(q  k^T),  P_lora = softmax(q  k'^T)
    and the KL isolated the key edit.

    rung27's target set is ('to_q','to_kv','to_out','sa_qkv','sa_out'), so to_q
    carries an adapter too:
        P_frozen = softmax(q  k^T),  P_lora = softmax(q' k'^T)
    Both sides of the dot product move. The KL therefore measures the TOTAL
    change to the attention map, which is a strictly larger quantity than
    rung26's. Its magnitude does NOT transfer, so beta must be re-probed --
    rung26's ratios (spot 3,800 / teapot2 7,233) are for a different quantity.

WHY THE QUERY HAS TO BE STASHED
    The hook that owns the keys is on cross_attn.to_kv, and it never sees q.
    modules.py computes to_q(x) BEFORE to_kv(context), so a hook on to_q fires
    first and can stash both the frozen and adapted query for the kv hook to use.
    That stash hook is installed whenever --kl is on, EVEN IF to_q carries no
    adapter (e.g. --targets kv+sa) -- otherwise the KL would silently find no
    query and contribute nothing.

SELF-ATTENTION IS ADAPTED BUT NOT PENALISED HERE.  rung27's sa_qkv/sa_out
adapters still train; this file constrains the cross-attention map only. The
self-attention KL is rung28's, a separate file.

BETA IS MEASURED, NEVER CHOSEN.  --kl-probe computes the KL but does NOT add it
to the loss, and logs mean KL and mean loss so beta comes from the ratio.
TRELLIS.2's first KL attempt used beta=1.0 and the term was ~0.1% of the
gradient; rung26's eight arms then showed that a correctly-weighted CROSS KL
costs 1.17-5.98 dB monotonically on two objects. This run asks the same question
with self-attention also adapted.

rung27's header follows.
--------------------------------------------------------------------------------
...
ap.add_argument('--kl', action='store_true',
                help='KL(P_lora||P_frozen) on the CROSS-attention map.')
ap.add_argument('--w-kl', type=float, default=0.0,
                help='beta for the CROSS-attention KL. SET IT FROM --kl-probe.')
ap.add_argument('--w-kl-self', type=float, default=0.0,
                help='beta for the SELF-attention KL. Independent of --w-kl: the '
                     'two maps have different magnitudes, so one weight on the '
                     'sum could not attribute the effect to a site.')
ap.add_argument('--kl-blocks', default='all', choices=['all','early','mid','late'])
ap.add_argument('--kl-voxels', type=int, default=0,
                help='query voxels sampled per call; 0 = all. Cross-attn is '
                     '[1999,16,1029] fp32 = 131 MB/block, so all 30 blocks is '
                     '~3.9 GB for lp_a. Subsampling QUERIES is an unbiased '
                     'estimate of the same mean.')
ap.add_argument('--kl-probe', action='store_true',
                help='compute the KL but DO NOT apply it; log mean KL and mean '
                     'loss so beta comes from the measured ratio.')
...
ap.add_argument('--context-window', type=int, default=1, choices=[1, 3, 5],
                help="THE ONE CHANGE OVER rung27. 1 = OFF, byte-identical to "
                     "rung27. 3 = cross-attention sees frames [f-1,f,f+1] "
                     "CONCATENATED (1029 -> 3087 tokens); 5 = [f-2..f+2]. There is "
...
_CFG = dict(variant='trellis2_backproj_lora', loss_region='rendered_gt',
            # rung29. These MUST be in the hash. Without them every beta of
            # an object hashed to ONE directory, the arms overwrote each
            # other's checkpoints, and GATE-resume then inherited the
            # previous arm's weights because mesh and targets matched.
            kl=bool(args.kl), w_kl=float(args.w_kl),
            w_kl_self=float(args.w_kl_self),
            kl_blocks=args.kl_blocks, kl_voxels=int(args.kl_voxels),
            kl_probe=bool(args.kl_probe),
            ...
            rung=(34 if (_HAS_CW and args.kl and not args.kl_probe
                         and (args.w_kl > 0 and args.w_kl_self > 0))
                  else 32 if _HAS_CW
                  else 30 if (_HAS_SA and args.kl and not args.kl_probe
                         and (args.w_kl > 0 and args.w_kl_self > 0))
                  else 29 if (_HAS_SA and args.kl and args.w_kl > 0
            ...
            **({} if not args.mcfm else dict(mcfm=args.mcfm)),
            # Conditional for the same reason as mcfm: with the window
            # off, this key is absent and the hash equals rung27's, so a
            # --context-window 1 run resumes from an existing rung27 run.
            **({} if not _HAS_CW else dict(context_window=args.context_window)),
...
# rung29 = a self-attn arm carrying an APPLIED cross-attention KL. A probe, or
# --w-kl 0, stays 27 so it remains a matched cell against rung27 itself.
# rung34 = the WIDE CONTEXT carrying both applied KLs. It degrades through 32 (wide
# context, no KL), 30 (KL both, no widening), 29 (cross KL only) and 27, so every
# degenerate setting stays a matched cell against the arm it degenerates to.
# Without the first two branches a wide-context run stamped itself rung30 and was
# indistinguishable from a run with no widening at all.
_RUNG = (34 if (_HAS_CW and args.kl and not args.kl_probe
                and args.w_kl > 0 and args.w_kl_self > 0)
         else 32 if _HAS_CW
         else 30 if (_HAS_SA and args.kl and not args.kl_probe
                and args.w_kl > 0 and args.w_kl_self > 0)
         else 29 if (_HAS_SA and args.kl and args.w_kl > 0 and not args.kl_probe)
...
_RTAG  = (('' if args.recon == 'l2' else f'_{args.recon}') + ('_lp' if args.lpips else '')
          ...
          + (f'_cw{args.context_window}' if _HAS_CW else ''))
...
# ── rung29: the cross-attention KL ───────────────────────────────────────────

# Collected ONLY inside the graded flow_eval. run_ode's prefix, the gates and
# evaluate() all run with collect False, so nothing outside the training
# gradient appends a term or holds a [1999,16,1029] graph.
# 'cross' and 'self' are SEPARATE lists: one weight on a merged list could
# not attribute the effect to a site, which is the point of this rung.
_KL = {'cross': [], 'self': [], 'collect': False, 'blocks': set(), 'q': {}}
...
def attn_kl_cross(ca, q_frozen, q_adapted, kv_frozen, kv_adapted):
    """
    KL( P_lora || P_frozen ) over the image tokens, meaned over voxels and heads.

    SHAPES (modules.py). The hooks see PLAIN TENSORS, not SparseTensors, because
    _linear calls module(x.feats) and re-wraps afterwards:
        q_*   [N, C]          N = voxel tokens, no batch dim
        kv_*  [B, Lkv, 2C]    context is dense, B = 1
    reshape (..., 2, H, D) and unbind(-3)[0] takes K.

    BOTH ARMS USE THEIR OWN q. rung26 could share one query because to_q was
    frozen there; here to_q is adapted, so pairing the adapted keys with the
    frozen query would measure a distribution the model never computes.
    """
    H = ca.num_heads
    D = q_frozen.shape[-1] // H

    def prep(q, kv):
        q = q.reshape(-1, H, D)
        k = kv.reshape(*kv.shape[:-1], 2, H, D).unbind(dim=-3)[0].reshape(-1, H, D)
        if getattr(ca, 'qk_rms_norm', False):
            q, k = ca.q_rms_norm(q), ca.k_rms_norm(k)
        return q, k

    qf, kf = prep(q_frozen, kv_frozen)
    qa, ka = prep(q_adapted, kv_adapted)

    if args.kl_voxels > 0 and qf.shape[0] > args.kl_voxels:
        sel = torch.randint(0, qf.shape[0], (args.kl_voxels,), device=qf.device)
        qf, qa = qf.index_select(0, sel), qa.index_select(0, sel)

    scale = 1.0 / math.sqrt(D)
    with torch.no_grad():
        lp_f = torch.log_softmax(
            torch.einsum('nhd,khd->nhk', qf.float(), kf.float()) * scale, dim=-1)
    lp_a = torch.log_softmax(
        torch.einsum('nhd,khd->nhk', qa.float(), ka.float()) * scale, dim=-1)
    return (lp_a.exp() * (lp_a - lp_f)).sum(-1).mean()


def attn_kl_self(sa, qkv_frozen, qkv_adapted):
    """
    KL( P_lora || P_frozen ) over the VOXEL tokens — the propagation graph.

    self_attn.to_qkv is a FUSED Linear(C, 3C); reshape (..., 3, H, D) and index
    0/1 gives q and k. rung27 adapts this matrix, so BOTH move and neither is
    shared with the frozen arm.

    v is index 2 and is never touched by this term: the adapter may still change
    WHAT voxels exchange, only not WHO they attend to.

    attn_mode is 'full' (structured_latent_flow.py:71) so the map is [N, H, N] —
    254 MB fp32 per block at N=1999. --kl-voxels subsamples QUERIES, which is an
    unbiased estimate of the mean over queries; keys stay complete so every
    sampled row is still a true distribution.
    """
    H = sa.num_heads
    N = qkv_frozen.shape[0]
    D = qkv_frozen.shape[-1] // (3 * H)
    ...
    qf, kf = qk(qkv_frozen)
    qa, ka = qk(qkv_adapted)
    if getattr(sa, 'qk_rms_norm', False):
        qf, kf = sa.q_rms_norm(qf), sa.k_rms_norm(kf)
        qa, ka = sa.q_rms_norm(qa), sa.k_rms_norm(ka)

    if args.kl_voxels > 0 and N > args.kl_voxels:
        sel = torch.randint(0, N, (args.kl_voxels,), device=qkv_frozen.device)
        qf, qa = qf.index_select(0, sel), qa.index_select(0, sel)

    scale = 1.0 / math.sqrt(D)
    with torch.no_grad():
        lp_f = torch.log_softmax(
            torch.einsum('nhd,mhd->nhm', qf.float(), kf.float()) * scale, dim=-1)
    lp_a = torch.log_softmax(
        torch.einsum('nhd,mhd->nhm', qa.float(), ka.float()) * scale, dim=-1)
    return (lp_a.exp() * (lp_a - lp_f)).sum(-1).mean()
...
def lora_ctx(flow_model, registry):
    ...
    for i, blk in enumerate(flow_model.blocks):
        ...
        def _q(mod, inp, out, _lb=lb, _i=i):
            adapted = out + _lb.to_q(inp[0]).to(out.dtype)
            # rung29. to_q fires BEFORE to_kv, so stash both queries here for the
            # kv hook, which owns the keys but never sees q.
            if _KL['collect'] and _i in _KL['blocks']:
                _KL['q'][_i] = (out, adapted)
            return adapted
        ...
        def _kv(mod, inp, out, _lb=lb, _blk=blk, _i=i):
            ...
            if _KL['collect'] and _i in _KL['blocks'] and _i in _KL['q']:
                qf, qa = _KL['q'].pop(_i)
                _KL['cross'].append(
                    attn_kl_cross(_blk.cross_attn, qf, qa, out, adapted))
            return adapted
        ...
        def _sq(mod, inp, out, _lb=lb, _blk=blk, _i=i):
            adapted = out + _lb.sa_qkv(inp[0]).to(out.dtype)
            # rung30. `out` is the frozen qkv and `adapted` the LoRA one, both in
            # hand, so no second forward is needed.
            if _KL['collect'] and _i in _KL['blocks']:
                _KL['self'].append(attn_kl_self(_blk.self_attn, out, adapted))
            return adapted
        ...
        elif args.kl and i in _KL['blocks']:
            # to_q is not a target (e.g. --targets kv+sa), so _q above is never
            # installed and _KL['q'] would stay empty and the KL silently vanish.
            # Stash the frozen query as BOTH arms: with no adapter on to_q they
            # are in fact identical, which is rung26's situation exactly.
            def _qs(mod, inp, out, _i=i):
                if _KL['collect'] and _i in _KL['blocks']:
                    _KL['q'][_i] = (out, out)
                return out
            handles.append(blk.cross_attn.to_q.register_forward_hook(_qs))
        if lb.has('to_kv'):
...
def main():
    ...
    # ------------------------------------------------------------------ rung32
    # WIDE CONTEXT. Hand cross-attention the window's tokens CONCATENATED instead
    # of one frame's. Nothing else changes: same LoRA set, same blocks, same
    # to_kv. The frozen to_kv maps a token to a key/value and is indifferent to
    # how many tokens it is given, so a longer bank needs no new matrices -- each
    # voxel simply attends over (patch, frame) jointly and resolves time itself.
    #
    # This is the OPPOSITE of MCFM. MCFM collapses the window to one conditioning
    # image BEFORE the model runs, fixing the temporal answer in advance; here
    # nothing is collapsed and the answer is learned, per block, by attention that
    # already exists. The two are mutually exclusive by design -- see the assert.
    #
    # Edge frames CLAMP to the nearest existing frame (frame 1's f-1 is frame 1),
    # matching mcfm_blend.blend_conds so boundary behaviour is comparable.
    if _HAS_CW:
        assert not args.mcfm, (
            '--context-window and --mcfm are mutually exclusive: MCFM pre-collapses '
            'the window that this flag exists to hand over intact. Running both '
            'would blend the frames and then concatenate the blends, which is '
            'neither method and answers no question.')
        _keys = sorted(conds.keys())
        _lo, _hi = _keys[0], _keys[-1]
        _half = args.context_window // 2
        _off = list(range(-_half, _half + 1))
        assert len(_off) == args.context_window, (_off, args.context_window)
        ...
        assert _shape[-2] == _n_img * args.context_window, _shape
        print(f'[R32] context_window={args.context_window} offsets={_off}  '
              f'tokens {_n_img} -> {_shape[-2]}', flush=True)

        # GATE-window: the CENTRE slice must be frame f, byte-for-byte. If the
        # ordering were wrong every frame would be conditioned on a neighbour and
        # the run would still train, still converge, and still be wrong -- the
        # exact failure mode that is invisible until the renders come back.
        _f = _keys[len(_keys) // 2]
        _mid = conds[_f][..., _half * _n_img:(_half + 1) * _n_img, :]
        assert torch.equal(_mid, _base[_f]), (
            'GATE-window FAILED: centre slice is not frame f')
        # and the flanks must be the NEIGHBOURS, not copies of f -- otherwise the
        # window is cosmetic and rung32 is rung27 with three times the compute.
        if _f - 1 >= _lo:
            _l = conds[_f][..., (_half - 1) * _n_img:_half * _n_img, :]
            assert torch.equal(_l, _base[_f - 1]), 'GATE-window FAILED: left flank'
            assert not torch.equal(_l, _base[_f]), (
                'GATE-window FAILED: left flank equals frame f — window is a no-op')
        print(f'[R32] GATE-window PASSED — centre == frame {_f}, flanks are '
              f'its neighbours', flush=True)
    ...
    for ep in range(start_ep, EPOCHS + 1):
        ...
        _klctot = _klstot = 0.0; _kln = 0   # rung30: the two KLs, separately
        ...
        for si, fi in enumerate(order, 1):
            ...
            with lora_ctx(flow, reg):
                ...
                _klc_terms = list(_KL['cross'])     # snapshot; replay re-appends
                _kls_terms = list(_KL['self'])
                _KL['cross'].clear(); _KL['self'].clear(); _KL['q'].clear()
                ...
                loss = loss_data
                _klcv = _klsv = 0.0
                if args.kl and _klc_terms:
                    _klm_c = torch.stack(_klc_terms).mean()
                    _klcv = float(_klm_c.detach())
                    assert math.isfinite(_klcv), f'frame {fi} k{k}: cross KL not finite'
                    if not args.kl_probe and args.w_kl > 0:
                        loss = loss + args.w_kl * _klm_c
                    del _klm_c
                if args.kl and _kls_terms:
                    _klm_s = torch.stack(_kls_terms).mean()
                    _klsv = float(_klm_s.detach())
                    assert math.isfinite(_klsv), f'frame {fi} k{k}: self KL not finite'
                    if not args.kl_probe and args.w_kl_self > 0:
                        loss = loss + args.w_kl_self * _klm_s
                    del _klm_s
                if args.kl and (_klc_terms or _kls_terms):
                    _klctot += _klcv; _klstot += _klsv; _kln += 1
                _lpv = 0.0
        ...
        print(f'[EPOCH {ep}/{EPOCHS}] loss={row["loss"]:.5f}  '
              ...
              + (f'  Rcon={_rctot/max(_rn,1):.3e}(x{args.lambda_con*_ed/max(_ec,1e-12):.1f})'
                 ...
                 if REG_ON else '')
              + (f'  KLc={_klctot/max(_kln,1):.4e}'
                 f'  KLs={_klstot/max(_kln,1):.4e}'
                 f'  loss/KLc={row["loss"]/max(_klctot/max(_kln,1),1e-30):.4g}'
                 f'  loss/KLs={row["loss"]/max(_klstot/max(_kln,1),1e-30):.4g}'
                 + ('  [PROBE: neither applied]' if args.kl_probe else
                    f'  shr_c={100*args.w_kl*(_klctot/max(_kln,1))/max(row["loss"],1e-30):.1f}%'
                    f'  shr_s={100*args.w_kl_self*(_klstot/max(_kln,1))/max(row["loss"],1e-30):.1f}%')
                 if args.kl and _kln else '')
