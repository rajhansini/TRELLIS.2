...
_CFG = dict(variant='trellis2_backproj_lora', loss_region='rendered_gt',
            ...
            rung=(33 if (_HAS_T and args.kl and not args.kl_probe
...
# rung33 = the dual branch carrying BOTH applied KLs. It degrades through 31 (no
# KL), 30 (KL both, no branch), 29 (cross KL only) and 27, so every degenerate
# setting stays a matched cell against the arm it degenerates to.
_RUNG = (33 if (_HAS_T and args.kl and not args.kl_probe
                and args.w_kl > 0 and args.w_kl_self > 0)
         else 31 if _HAS_T
         else 30 if (_HAS_SA and args.kl and not args.kl_probe
                     and args.w_kl > 0 and args.w_kl_self > 0)
...
def lora_ctx(flow_model, registry):
    ...
    for i, blk in enumerate(flow_model.blocks):
        ...
        # _p is bound as a DEFAULT, not captured by closure: _pick is rebuilt every
        # loop iteration, so a closure would late-bind all 30 hooks to the last
        # block's bundles.
        #
        # KL IS COLLECTED ON THE SPATIAL PASS ONLY (_BR['cur'] == 'S'). The temporal
        # pass attends over a 3x longer context, so its attention map has different
        # support and is not comparable to the frozen model's; summing the two would
        # add two incommensurable KLs into one number and the weight would mean
        # nothing.
        def _q(mod, inp, out, _p=_pick, _i=i):
            b = _p('to_q')
            adapted = out if b is None else out + b.to_q(inp[0]).to(out.dtype)
            if _KL['collect'] and _BR['cur'] == 'S' and _i in _KL['blocks']:
                _KL['q'][_i] = (out, adapted)
            return adapted

        def _kv(mod, inp, out, _p=_pick, _xa=None, _i=i):
            b = _p('to_kv')
            adapted = out if b is None else out + b.to_kv(inp[0]).to(out.dtype)
            if (_KL['collect'] and _BR['cur'] == 'S'
                    and _i in _KL['blocks'] and _i in _KL['q']):
                qf, qa = _KL['q'].pop(_i)
                _KL['cross'].append(attn_kl_cross(_xa, qf, qa, out, adapted))
            return adapted
        ...
        _kv.__defaults__ = (_pick, _xa, i)      # bind the unwrapped module for KL
        ...
        if _hasany('to_q'):
            ...
            # to_q carries no adapter, so _q never installs and _KL['q'] stays empty
            # -- the cross KL would silently vanish. Stash the frozen query as BOTH
            # arms; with no adapter on to_q they are in fact identical.
            def _qs(mod, inp, out, _i=i):
                if _KL['collect'] and _BR['cur'] == 'S' and _i in _KL['blocks']:
                    _KL['q'][_i] = (out, out)
                return out
            handles.append(_xa.to_q.register_forward_hook(_qs))
        if _hasany('to_kv'):
