...
ap.add_argument('--w-kl', type=float, default=0.0,
                help='beta. SET IT FROM --kl-probe. rung26 ratios do not transfer: '
                     'q moves here and it did not there.')
...
_CFG = dict(variant='trellis2_backproj_lora', loss_region='rendered_gt',
            ...
            rung=(29 if (_HAS_SA and args.kl and args.w_kl > 0
...
_RUNG = (29 if (_HAS_SA and args.kl and args.w_kl > 0 and not args.kl_probe)
...
_KL = {'terms': [], 'collect': False, 'blocks': set(), 'q': {}}
...
def main():
    ...
    for ep in range(start_ep, EPOCHS + 1):
        ...
        _kltot, _kln = 0.0, 0         # rung29 cross-attention KL
        ...
        for si, fi in enumerate(order, 1):
            ...
            with lora_ctx(flow, reg):
                ...
                _kl_terms = list(_KL['terms'])      # snapshot; replay may re-append
                _KL['terms'].clear(); _KL['q'].clear()
                ...
                if args.kl and _kl_terms:
                    _klm = torch.stack(_kl_terms).mean()
                    _klv = float(_klm.detach())
                    assert math.isfinite(_klv), f'frame {fi} k{k}: KL not finite'
                    if not args.kl_probe and args.w_kl > 0:
                        loss = loss + args.w_kl * _klm
                    del _klm
                    _kltot += _klv; _kln += 1
                _lpv = 0.0
        ...
        print(f'[EPOCH {ep}/{EPOCHS}] loss={row["loss"]:.5f}  '
              ...
              + (f'  Rcon={_rctot/max(_rn,1):.3e}(x{args.lambda_con*_ed/max(_ec,1e-12):.1f})'
                 ...
                 if REG_ON else '')
              + (f'  KL={_kltot/max(_kln,1):.4e}'
                 f'  loss/KL={row["loss"]/max(_kltot/max(_kln,1),1e-30):.4g}'
                 + ('  [PROBE: not applied]' if args.kl_probe else
                    f'  share={100*args.w_kl*(_kltot/max(_kln,1))/max(row["loss"],1e-30):.1f}%')
