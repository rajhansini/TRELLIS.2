"""
rung28_kl_selfattn.py — rung27 + a KL on the SELF-ATTENTION distribution
================================================================================
NEW FILE, copied from rung27_selfattn_lora.py. rung27 is untouched.
--w-kl 0 reproduces rung27 exactly.

WHAT THE TERM IS
    KL( P_lora || P_frozen )   over the 1,999 voxels, meaned over query voxels
    and heads, per adapted block.
        P = softmax( q k^T / sqrt(d) )
    loss = L1 + 0.1*LPIPS + w_kl * KL

WHY SELF-ATTENTION AND NOT CROSS
    rung27's recorded prediction is front-view UP, off-axis DOWN, because the
    loss sees one camera and adapting the distributor lets gradient descent
    reroute texture toward the front. The self-attention map IS that
    distributor -- who talks to whom among voxels. A KL on it penalises exactly
    the rerouting rung27 predicts, while leaving v free: the adapter may still
    change WHAT voxels exchange, only not WHO they attend to.

    On cross-attention the same term was measured and it only ever cost:
    teapot_lava2 19.688 -> 17.864 / 16.832 / 14.767 / 13.710 dB across two
    decades of beta, monotone, no minimum. --kl-site cross reproduces that arm.

BOTH q AND k MOVE HERE.  self_attn.to_qkv is a FUSED Linear(1536, 4608); rung27
adapts it, so the adapted q and k both enter the softmax. That differs from the
cross-attention arms, where --targets kv moved k alone and q was shared between
the two arms. Here neither is shared and the KL sees the full change.

MEMORY.  Self-attention is attn_mode='full' (structured_latent_flow.py:71), so
the map is [1999, 16, 1999] fp32 = 256 MB per block -- 30 blocks would be 7.7 GB
for lp_a alone, before the graph. --kl-blocks and --kl-voxels bound it; the probe
reports peak GPU so the choice is measured, not guessed.

BETA IS MEASURED, NEVER CHOSEN.  --kl-probe trains with the KL computed but NOT
applied, logs mean KL and mean loss, and prints the beta for a target share.
TRELLIS.2's earlier cross-attn runs used beta=1.0 and the term was ~0.1% of the
gradient -- those runs tested nothing.
...
ap.add_argument('--kl', action='store_true',
                help='KL(P_lora||P_frozen) on the attention map.')
ap.add_argument('--w-kl', type=float, default=0.0,
                help='beta. SET IT FROM --kl-probe. TRELLIS.2 used 1.0 on the '
                     'cross-attn arm and the term was ~0.1%% of the gradient.')
ap.add_argument('--kl-site', default='self', choices=['self', 'cross', 'both'],
                help="self = the voxel->voxel map rung27 adapts (the distributor). "
                     "cross = the voxel->image map, already measured and it only "
                     "cost 1.8-6.0 dB.")
ap.add_argument('--kl-blocks', default='all', choices=['all','early','mid','late'])
ap.add_argument('--kl-voxels', type=int, default=256,
                help='query voxels sampled per call. Self-attn is full: the map '
                     'is [1999,16,1999] fp32 = 256 MB/block, so all 30 blocks at '
                     'full width would be 7.7 GB for lp_a alone. Subsampling '
                     'QUERIES is an unbiased estimate of the same mean. 0 = all.')
ap.add_argument('--kl-probe', action='store_true',
                help='compute the KL but DO NOT add it to the loss; log mean KL '
                     'and mean loss so beta comes from the measured ratio.')
...
# rung28 = any self-attn arm that also carries the KL. Resolved after _RUNG is
# computed, so a --w-kl 0 run is still labelled 27 and stays comparable.
...
# rung28 = a self-attn arm carrying an APPLIED KL. A probe, or --w-kl 0, stays
# labelled 27 so it remains a matched cell against rung27 itself.
_RUNG = (28 if (_HAS_SA and args.kl and args.w_kl > 0 and not args.kl_probe)
...
# ── rung28: the attention-distribution KL ─────────────────────────────────────

# Collected ONLY inside the graded flow_eval. run_ode's prefix, every gate and
# every evaluate() run with collect False, so nothing outside the training
# gradient appends a term or holds a [1999,16,1999] graph.
_KL = {'terms': [], 'collect': False, 'blocks': set(), 'site': 'self'}
_KLB = {'all':   list(range(_NB)),
        'early': list(range(0, _THIRD)),
        'mid':   list(range(_THIRD, 2 * _THIRD)),
        'late':  list(range(2 * _THIRD, _NB))}[args.kl_blocks]
_KL['blocks'] = set(_KLB)
_KL['site']   = args.kl_site


def attn_kl_self(sa, qkv_frozen, qkv_adapted):
    """
    KL( P_lora || P_frozen ) over the voxels, meaned over query voxels and heads.

    self_attn.to_qkv is a FUSED Linear(C, 3C); reshape (..., 3, H, D) and unbind
    gives q, k, v in that order. rung27 adapts this matrix, so BOTH q and k move
    -- unlike the cross-attention arms where --targets kv moved k alone and q was
    shared between the two arms. Nothing is held fixed here and the KL sees the
    whole change to the map.

    v is never touched by this term: the adapter may still change WHAT voxels
    exchange, only not WHO they attend to.

    QUERIES ARE SUBSAMPLED. attn_mode is 'full', so the map is [N, H, N]; at
    N=1999, H=16 that is 256 MB fp32 per block. Sampling --kl-voxels queries is
    an unbiased estimate of the mean over queries, and keys stay complete so each
    sampled row is still a true distribution over all voxels.
    """
    H = sa.num_heads
...
def lora_ctx(flow_model, registry):
    ...
    for i, blk in enumerate(flow_model.blocks):
        ...
        def _sq(mod, inp, out, _lb=lb, _blk=blk, _i=i):
            ...
            # rung28. `out` IS the frozen qkv and `adapted` the LoRA one, both in
            # hand, so the KL needs no second forward pass.
            if _KL['collect'] and _KL['site'] in ('self','both') and _i in _KL['blocks']:
                _KL['terms'].append(attn_kl_self(_blk.self_attn, out, adapted))
            return adapted
...
def main():
    ...
    for ep in range(start_ep, EPOCHS + 1):
        ...
        _kltot, _kln = 0.0, 0         # rung28 attention KL, logged every epoch
        ...
        for si, fi in enumerate(order, 1):
            ...
            with lora_ctx(flow, reg):
                ...
                if args.kl and _kl_terms:
                    ...
                    # --kl-probe MEASURES without applying, so beta is set from
                    # the ratio instead of guessed.
                    if not args.kl_probe and args.w_kl > 0:
                        loss = loss + args.w_kl * _klm
                    del _klm
                _kltot += _klv; _kln += 1 if _kl_terms else 0
                _lpv = 0.0
        ...
        print(f'[EPOCH {ep}/{EPOCHS}] loss={row["loss"]:.5f}  '
              ...
              + (f'  KL={_kltot/max(_kln,1):.4e}'
                 f'  ratio=loss/KL={row["loss"]/max(_kltot/max(_kln,1),1e-30):.4g}'
                 + (f'  [PROBE: not applied]' if args.kl_probe else
