...
# OBJECT-SCOPED VARIANT. The original globs out/FULLRATE/*.json, which silently
# blends every batch into one table -- exactly what a per-batch table must not do.
# Here the object list is explicit and nothing is written over the A-C outputs.
import argparse as _ap
_p=_ap.ArgumentParser()
_p.add_argument('--objs',required=True,help='comma-separated object list')
_p.add_argument('--label',required=True,help='table label, e.g. batchDE')
_p.add_argument('--gate',default='',help='optional published per_cell json to gate against')
_p.add_argument('--dir',default='FULLRATE',help='FULLRATE (raw-frame GT) or FULLRATE_CG (2D-copy GT)')
_A=_p.parse_args()
WANT=[o for o in _A.objs.split(',') if o]
DIR=_A.dir
raw={}
for o in WANT:
    f=f'{E}/out/{DIR}/{o}.json'
    if not os.path.exists(f): print(f'  missing FULLRATE json: {o}'); continue
    raw.update(json.load(open(f)))
if not raw: sys.exit('no FULLRATE jsons for the requested objects')
pub=json.load(open(_A.gate))['per_cell'] if _A.gate else {}
...
print(f'objects: {len(raw)}/{len(WANT)}')
...
if not pub: print('validation gate: SKIPPED (no --gate given)')
...
json.dump({'n_objects':len(objs),'objects':objs,'rows':rows},
          open(f'{E}/out/fullrate_table_{_A.label}.json','w'),indent=1)
print(f'\n-> {E}/out/fullrate_table_{_A.label}.json')
