#!/bin/bash
# submit_w11_renders.sh -- pixel renders of the W=11 arm (mcfm v2_G) for all 42
...
for OBJ in $OBJS; do
  for SPEC in "27g:v2_G"; do
    ...
    for V in "train:0.0:0.0" "diagA:45.0:25.0" "diagB:135.0:-20.0" "diagC:225.0:30.0"; do
      ...
      grep -qx "w11_${OBJ}_${VIEW}" /tmp/_dl.$$ && continue
      ...
      J=$(sbatch --parsable ${NICE:+--nice="$NICE"} --job-name="w11_${OBJ}_${VIEW}" \
           --export=ALL,"$EXP" "$E/jobs/render_arm.sbatch")
      note "$J" "w11_${OBJ}_${VIEW}" "$EXP" "jobs/render_arm.sbatch" \
