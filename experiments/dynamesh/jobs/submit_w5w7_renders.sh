#!/bin/bash
# submit_w5w7_renders.sh -- pixel renders of the W=5 (v2_E) and W=7 (v2_F) arms, all 42
...
cd "$E"
OBJS="ancient_lady_effect_2 animal_blob_crack animal_blob_orange_crack chair_ice chair_moss chair_real_wooden_crack dragon_mush dragon_mush2 fish_glitter fish_ink hand_rorschach napolean_teapot_crack napolean_waves octocat_clay octocat_shine octopus_rainbow octopus_rust octopus_sparkle octopus_tar pumpkin_rot sheep_mud sheep_soil spot_lava tie_fighter_bw moai_silver moai_animated gargoyle_spiral gargoyle_effect_one airplane_blub airplane_red_cracks teddy_bleach teddy_fusion goat_burnt goat_flower goat_clay ivysaur_petal ivysaur_petal_2 blub_raurshaw blub_drying pegasus_shine pegaso_effect_1 mosaic_painting"
...
N=0; S=0; MISS=""
for OBJ in $OBJS; do
  for SPEC in "27e:v2_E" "27f:v2_F"; do
    TAG=${SPEC%%:*}; MODE=${SPEC##*:}
    case "$TAG" in 27e) PFX=w5 ;; 27f) PFX=w7 ;; *) PFX=wX ;; esac
    ...
    for V in "train:0.0:0.0" "diagA:45.0:25.0" "diagB:135.0:-20.0" "diagC:225.0:30.0"; do
      ...
      [ -d "$OUTD/frames" ] && [ "$(ls "$OUTD/frames"/*.png 2>/dev/null | wc -l)" -ge "$(awk -F"\t" -v o="$OBJ" '$2==o{print $3}' "$E/jobs/rung37_objects.tsv")" ] && continue
      grep -qx "${PFX}_${OBJ}_${VIEW}" /tmp/_dl.$$ && continue
      if [ -n "${DRY:-}" ]; then printf '   %-24s %-4s %-6s yaw=%-6s elev=%s\n' "$OBJ" "$TAG" "$VIEW" "$YAW" "$ELEV"; continue; fi
      # NFR is per-object: pumpkin_rot is 121 frames, every other object is 150.
      NFR=$(awk -F"\t" -v o="$OBJ" '$2==o{print $3}' "$E/jobs/rung37_objects.tsv")
      NFR=${NFR:-150}
      EXP="RUN=${RUN},TAG=view_${OBJ}_${TAG}_${VIEW},YAW0=${YAW},ELEV=${ELEV},TURNS=0,NFR=${NFR},RES=518"
      J=$(sbatch --parsable ${NICE:+--nice="$NICE"} --job-name="${PFX}_${OBJ}_${VIEW}" \
           --export=ALL,"$EXP" "$E/jobs/render_arm.sbatch")
      note "$J" "${PFX}_${OBJ}_${VIEW}" "$EXP" "jobs/render_arm.sbatch" \
