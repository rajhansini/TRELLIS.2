OBJ=$1; MESH=$2; GTDIR=$3; TGT=$4; NF=$5; EP=$6
cat <<SB
#!/bin/bash
#SBATCH --job-name=r29p_${OBJ}
...
#SBATCH --output=/net/projects/ranalab/rajhansini/TRELLIS.2/experiments/dynamesh/out/r29probe_${OBJ}_%j.log
#SBATCH --error=/net/projects/ranalab/rajhansini/TRELLIS.2/experiments/dynamesh/out/r29probe_${OBJ}_%j.log
set -eo pipefail
...
\${PY2} -u "\${E}/rung29_kl_cross_attention.py" \\
  --mesh "${MESH}" \\
  --gt-dir "${GTDIR}" \\
  --gt-render-dir "${TGT}" \\
  ...
  --kl --kl-blocks all --kl-probe --w-kl 0 \\
  --epochs ${EP} --n-frames ${NF} --seed 42 --lr 1e-4 \\
