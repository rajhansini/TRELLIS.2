...
ARM='27m'   # overridden by --arm; see frame_paths()
...
def frame_paths(obj,view,n):
    ...
    for f in range(1,n+1):
        ...
        arm_m=E/f'view_{obj}_{ARM}_{view}'/'frames'/f'{f:04d}.png'
...
def run_object(obj):
    ...
    for view in VIEWS:
        ...
        for m in ms:
            ...
            with ThreadPool(int(os.environ.get('FR_THREADS',16))) as tp:   # NFS latency, not CPU
...
if __name__=='__main__':
    ...
    ap.add_argument('--arm',default='27m',
                    help="render tag for the adapted panel: 27m=W3(v2_D), "
                         "27e=W5(v2_E), 27f=W7(v2_F), 27g=W11(v2_G). The frozen "
                         "panel always comes from view_<obj>_27_<view>, which is "
                         "the non-MCFM run -- see the note above frame_paths().")
    a=ap.parse_args()
    globals()['ARM']=a.arm      # frame_paths() reads this module global
