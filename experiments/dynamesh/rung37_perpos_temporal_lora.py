"""
rung37_perpos_temporal_lora.py — LEARNED temporal attention, restricted to the
SAME SPATIAL POSITION across the window.
================================================================================
NEW FILE, copied from rung31_dual_attn_lora.py. rung31 is not modified.

WHY THIS RUNG EXISTS — rung31 IS NOT A FAIR CONTROL FOR MCFM
  MCFM v2_D blends conditioning token i over the window and touches NOTHING else:
  1029 independent softmaxes, each over W items, no spatial term. rung31's
  temporal pass hands the flow the WHOLE stacked window (W*1029 tokens) with no
  mask, so every voxel query attends over every spatial position in every frame.
  That is v3 geometry (joint spatio-temporal), not v2.

  So "rung31 loses to MCFM v2_D" conflates two axes at once -- learned vs free,
  AND joint vs per-position. Measured, the joint free operator (v3_D) reaches
  flicker 0.00575 against v2_D's 0.00684, so geometry alone moves the metric more
  than the learned/free axis does. The comparison the paper needs is a LEARNED
  operator with v2 geometry. That is this file.

WHAT CHANGES vs rung31
  Only the temporal pass. Instead of cross-attending over W*N tokens, the window
  is folded to (N, W, D) and a learned per-position attention collapses the W
  axis to one token per position -- exactly mcfm_blend's v2, with trained Q/K
  projections replacing its fixed dot product. The resulting N tokens then go
  through the same frozen cross-attention, carrying rung31's separate temporal
  LoRA slot. The spatial pass, the gate, the branch flag and every diagnostic are
  untouched.

      pass S : context = frame f alone                 (1029 tokens)  <- rung27
      pass T : context = per-position mix of the window (1029 tokens) <- NEW
      out    = S + gate * T

  gate_init=0.0 keeps step 0 bit-identical to rung27, as in rung31.

WHAT MAKES IT THE MATCHED CONTROL
  Same backbone, same rank, same seed, same schedule, same window, same gate, and
  now the same ATTENTION GEOMETRY as MCFM v2_D. The only remaining difference is
  that the temporal combination is learned instead of a fixed softmax, which is
...
_CFG = dict(variant='trellis2_backproj_lora', loss_region='rendered_gt',
            ...
            rung=(37 if _HAS_T else 27 if _HAS_SA
            ...
            **({} if not args.mcfm else dict(mcfm=args.mcfm)),
            ...
            # temporal_geometry is in the hash on purpose: rung31 and rung37 share
            # every other config field, so without it the two would land in the same
            # directory and resume from each other's checkpoints -- the exact failure
            # the --mcfm hash entry was added to prevent.
            **({} if not _HAS_T else dict(temporal_geometry='per_position',
...
_RUNG = (37 if _HAS_T else 27 if _HAS_SA          # keep byte-identical to _CFG['rung'] above
         else 24 if (args.conf_ramp_qlo >= 0 and args.conf_ramp_qhi >= 0)
         else 22 if (args.lambda_con > 0 and args.lambda_smo <= 0)
         else 21 if (args.lambda_con > 0 or args.lambda_smo > 0)
...
_RTAG  = (('' if args.recon == 'l2' else f'_{args.recon}') + ('_lp' if args.lpips else '')
          ...
          + (f'_tw{args.temporal_window}{args.branch[0]}pp' if _HAS_T else ''))
...
class PerPosTemporalMix(nn.Module):
    """Collapse a (..., W, N, D) window to (..., N, D), per position.

    Token i attends over token i in the W frames and over NOTHING else. This is
    mcfm_blend.blend_window's v2 branch with learned projections:

        MCFM v2 :  alpha = softmax_o( <c_t[i], c_{t+o}[i]> / sqrt(D) )
        here    :  alpha = softmax_o( <Q c_t[i], K c_{t+o}[i]> / sqrt(D) )

    Q and K are LoRA-style low-rank deltas on the identity, B zero-init, so at
    step 0 Q = K = I and this module is EXACTLY MCFM v2 with the same window and
    the same clamping. Training can only move it away from that point, which
    makes MCFM the initialisation of the learned operator rather than a separate
    baseline -- the cleanest possible control.

    No value projection: the output is a convex combination of the ORIGINAL
    tokens, as in MCFM. Adding one would change what is being mixed as well as
    how, and the question here is only 'how'.
    """

    def __init__(self, d, rank, alpha=4.0):
        super().__init__()
        self.d = int(d)
        self.q = LoRALayer(d, d, rank, alpha)
        self.k = LoRALayer(d, d, rank, alpha)

    def forward(self, win, mid):
        # win: (..., W, N, D)   mid: index of frame f inside the window
        cur = win[..., mid, :, :]                      # (..., N, D)
        q = cur + self.q(cur)                          # (..., N, D)   B=0 -> q=cur
        k = win + self.k(win)                          # (..., W, N, D)
        # per position: <q[i], k[o,i]> over D, for each o
        logits = (k * q.unsqueeze(-3)).sum(-1) * (self.d ** -0.5)   # (..., W, N)
        a = torch.softmax(logits, dim=-2).unsqueeze(-1)             # (..., W, N, 1)
        return (a * win).sum(-3)                                    # (..., N, D)
...
class LoRARegistry(nn.Module):
    def __init__(self, n_blocks, ch, ctx, rank, with_mlp, mlp_hidden,
        ...
        if self.dual:
            ...
            self.gates = nn.ParameterDict({
                ...
                for i in self.active})
            # rung37: one per-position temporal mixer per block. Lives in the
            # registry so it lands in reg.parameters() and is picked up by the
            # optimizer, the checkpoint and grad clipping with no extra wiring --
            # the same reason the gates live here.
            self.tmix = nn.ModuleDict({
                str(i): PerPosTemporalMix(ctx, rank, alpha) for i in self.active})
    ...
    def mix(self, i):
        k = str(i)
        return self.tmix[k] if (self.dual and k in self.tmix) else None
...
class DualCrossAttn(nn.Module):
    """Wraps ONE block's cross_attn and runs it twice.
        ...
        pass S : context = frame f alone                  (1029 tokens) <- rung27
        pass T : context = PER-POSITION mix of the window (1029 tokens) <- rung37
        out    = S + gate * T

    rung31 passed the raw W*1029 window here, which is joint spatio-temporal
    attention. rung37 collapses the window per position first, so the temporal
    branch has MCFM v2_D's geometry and differs from it only by being learned.
    ...
    def forward(self, x, context, *a, **kw):
        ...
        _BR['cur'] = 'T'
        # rung37: fold (..., W*N, D) -> (..., W, N, D) and collapse the W axis
        # PER POSITION, so the temporal pass sees n_img tokens, not W*n_img.
        # unflatten keeps frame-major order, matching how _stack concatenated it.
        win = context.unflatten(-2, (W, n))
        c_mix = self._reg[0].mix(self.idx)(win, mid)
        ot = self.inner(x, c_mix, *a, **kw)
        _BR['cur'] = 'S'
...
def main():
    ...
    if _HAS_T:
        ...
        print(f'[R37] per-position window={args.temporal_window} offsets={_off}  '
        ...
        print(f'[R37] GATE-window PASSED — middle slice == frame {_f} exactly', flush=True)

        for _i in ACTIVE:
            flow.blocks[_i].cross_attn = DualCrossAttn(
                flow.blocks[_i].cross_attn, _n_img, reg, _i)
        print(f'[R37] DualCrossAttn installed on {len(ACTIVE)} blocks', flush=True)
