...
ap.add_argument('--targets',    default='qkv',
                choices=['kv', 'qkv', 'qkvo', 'cross', 'cross+mlp'],
                help="kv   = to_kv ONLY. With the mesh and the noise both fixed, "
...
import hashlib
_CFG = dict(variant='trellis2_intersection_lora', loss_region='intersection', blocks=args.blocks,
            active=ACTIVE, target_set=list(_TSET), rank=args.rank, targets=args.targets,
...
LABEL  = f'rung16_{args.blocks}_{args.targets}_r{args.rank}_s{args.seed}_{RUN_ID}'
...
class CrossAttnLoRA(nn.Module):
    ...
    def __init__(self, ch, ctx, rank, targets=('to_q', 'to_kv', 'to_out'),
                 with_mlp=False, mlp_hidden=None):
        super().__init__()
        if 'to_q' in targets:
...
class LoRARegistry(nn.Module):
    def __init__(self, n_blocks, ch, ctx, rank, with_mlp, mlp_hidden,
                 targets=('to_q', 'to_kv', 'to_out'), active=None):
        super().__init__()
        self.targets = tuple(targets)
        self.active = list(range(n_blocks)) if active is None else list(active)
        self.blocks = nn.ModuleDict({
            str(i): CrossAttnLoRA(ch, ctx, rank, self.targets, with_mlp, mlp_hidden)
...
def main():
    ...
    reg = LoRARegistry(nb, ch, ctx, args.rank,
                       ...
                       targets=_TSET, active=ACTIVE).to(DEVICE)
    ...
    for fi, r, a in zip(frames, raws, alphas):
        ...
        # The GT silhouette is NOT constant: measured over the 150 frames it
        # grows 102,549 -> 109,583 px (+6.60%), height 328 -> 340. Our render is
        # fixed at 28,559 px because the mesh and camera are. Keep each frame's
        # own object mask so the loss can be taken on the INTERSECTION.
        # `a` is already this frame's largest-component object mask at full
        # resolution — reuse it rather than recomputing (and `args.bg_thresh`
        # does not exist in this script; the threshold lives in the `alphas`
        # comprehension above).
        gt_masks[fi] = torch.from_numpy(
            np.array(Image.fromarray((a * 255).astype(np.uint8))
                     .resize((args.render_res, args.render_res), Image.NEAREST)) > 127
        ).to(DEVICE)
    ...
    def inter_mask(fi):
        ...
        The complement is the mismatch band, and it is pure shape disagreement:
        one side has surface there and the other has background. Including it
        charges the adapter for a 6.6% time-varying silhouette difference as if
        it were texture error. rung13's alignment cannot remove it — that solved
        ONE static transform and this mismatch grows with the frame index.
        """
    ...
    _sizes = [int(inter_mask(f).sum()) for f in HELD_OUT]
    print(f'\n[REGION] loss/eval taken on render AND gt (per frame).')
    print(f'  render mask (fixed)      {int(renderer.mask.sum()):,} px')
    print(f'  intersection over held   min {min(_sizes):,}  max {max(_sizes):,}  '
          f'mean {sum(_sizes)//len(_sizes):,}')
    print(f'  NOTE PSNR here is NOT comparable to a render-mask PSNR (different '
          f'denominator); report delta over frozen, which is region-invariant.',
          flush=True)
    ...
    for ep in range(start_ep, EPOCHS + 1):
        ...
        for si, fi in enumerate(order, 1):
            ...
            with lora_ctx(flow, reg):
                ...
                loss = ((img - gts[fi]) ** 2)[:, inter_mask(fi)].mean()
