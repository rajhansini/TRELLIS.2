"""
rung31_warmstart_lora.py — rung27, started from ANOTHER OBJECT'S adapter.

Two changes over rung27; nothing else is touched.

  --init-from <ckpt>   start from a LoRA fit on a DIFFERENT object instead of
                       from B=0. GATE-plain still runs FIRST against B=0, so the
                       identity property is proved before the warm start lands.

  --crop-from-mask     conditioning crop box from render_mask.npy instead of a
                       brightness threshold.

WHY --crop-from-mask EXISTS. rung27 answers "where is the object" two different
ways, and on some videos they disagree:

    targets + loss mask   ->  gt_render_dir/render_mask.npy      (geometry)
    conditioning crop     ->  min(RGB) < 245 on the raw frames   (brightness)

Union of the brightness mask over each video, measured:

    spot_lava           21.2% of frame   crop 721 px    agree
    horse_metal         16.1%            crop 717 px    agree
    teapot_porcelain    16.4%            crop 716 px    agree
    penguin_circuits   100.0%            crop 959 px    <-- the whole frame
    whale_spots        100.0%            crop 959 px    <-- the whole frame

penguin and whale light their own background mid-video, so "not near-white"
becomes the entire image and the crop stops cropping. Nothing raises -- the
object is just conditioned at a different scale from every other run.
render_mask.npy holds the exact silhouette the targets were built on, and mesh
and camera never move, so it is the right source for the box.

Copied from rung27_selfattn_lora.py, which is NOT modified (md5 verified).

--- inherited header from rung27_selfattn_lora.py ---
LoRA on SELF-ATTENTION, in addition to cross-attention
================================================================================
...
ap.add_argument('--init-from', default=None,
                help='checkpoint of a LoRA trained on a DIFFERENT object to start '
                     'from. Rank and target set must match.')
ap.add_argument('--crop-from-mask', action='store_true',
                help='conditioning crop box from render_mask.npy rather than a '
                     'brightness threshold. Required when the background is not '
                     'near-white.')
...
_CFG = dict(variant='trellis2_backproj_lora', loss_region='rendered_gt',
            ...
            **({} if not args.init_from else dict(init_from=str(args.init_from))),
            **({} if not args.crop_from_mask else dict(crop_from_mask=True)),
...
def main():
    ...
    raws = [np.array(Image.open(Path(args.gt_dir) / f'frame_{fi:04d}.png').convert('RGB'))
            for fi in frames]
    if args.crop_from_mask:
        # Geometry, not brightness -- the same silhouette the targets were built
        # on. Mesh and camera are fixed, so one mask covers every frame.
        _cm = Path(args.gt_render_dir).parent / 'render_mask.npy'
        assert _cm.exists(), f'--crop-from-mask: {_cm} missing'
        union = np.load(_cm).astype(bool)
        # The same mask is also the per-frame ALPHA MATTE. It premultiplies the
        # conditioning image, so whatever it excludes goes black. With the
        # brightness matte, penguin's lit background survives into the
        # conditioning; with this one it does not. Mesh and camera are fixed and
        # GATE-align reports 0 disagreeing pixels, so one matte is right for
        # every frame.
        alphas = [union] * len(raws)
        _src = 'render_mask.npy'
    else:
        alphas = [largest_component(r.min(axis=2) < 245) for r in raws]
        union = np.zeros_like(alphas[0])
        for a in alphas:
            union |= a
        _src = 'brightness threshold'
    print(f'[CROP] source {_src} -- {int(union.sum()):,} px '
          f'({100 * union.sum() / union.size:.1f}% of frame)', flush=True)
    ys, xs = np.where(union)
    cx, cy = (xs.min() + xs.max()) / 2, (ys.min() + ys.max()) / 2
    sz = int(max(xs.max() - xs.min(), ys.max() - ys.min()))
    bbox = (int(cx - sz // 2), int(cy - sz // 2), int(cx + sz // 2), int(cy + sz // 2))

    # GATE-crop. A crop that is the whole frame is not a crop: the object gets
    # conditioned at a different scale from every other run, and nothing errors.
    _side = int(np.asarray(raws[0]).shape[0])
    print(f'[GATE-crop] bbox {bbox}  size {sz} px of a {_side} px frame '
          f'({100 * sz / _side:.0f}%)', flush=True)
    assert sz < 0.94 * _side, (
        f'GATE-crop FAILED: crop is {sz}/{_side} px, effectively the whole frame -- '
        f'the object was not localised. Pass --crop-from-mask to take the box from '
        f'render_mask.npy instead of a brightness threshold.')
    print('[GATE-crop] PASSED -- object is localised', flush=True)
    ...
    # ── GATE-init: the warm start ────────────────────────────────────────────
    # Applied HERE, not earlier, on purpose: GATE-plain immediately above has
    # just proved the adapter is the exact identity at B=0. Only then is another
    # object's solution written over it, so a warm-started run keeps that proof.
    if args.init_from:
        _ip = Path(args.init_from)
        assert _ip.exists(), f'GATE-init FAILED: {_ip} does not exist'
        _ist = torch.load(_ip, map_location=DEVICE, weights_only=False)
        _isd = _ist['reg'] if 'reg' in _ist else _ist
        _icfg = _ist.get('cfg', {}) or {}
        _mine = reg.state_dict()
        if set(_isd) != set(_mine):
            raise SystemExit(f'GATE-init FAILED: checkpoint has {len(_isd)} tensors, '
                             f'this registry has {len(_mine)} -- rank or target set differs')
        _bad = [k for k in _mine if _isd[k].shape != _mine[k].shape]
        if _bad:
            raise SystemExit(f'GATE-init FAILED: shape mismatch on {len(_bad)} tensors, '
                             f'first {_bad[0]}')
        _b4 = float(torch.cat([v.flatten() for k, v in _mine.items()
                               if k.endswith('.B')]).abs().max())
        reg.load_state_dict(_isd)
        _af = float(torch.cat([v.flatten() for k, v in reg.state_dict().items()
                               if k.endswith('.B')]).abs().max())
        print(f'\n[GATE-init] source    : {_ip}')
        print(f'[GATE-init] trained on: {_icfg.get("mesh", "?")}')
        print(f'[GATE-init] targets   : {_icfg.get("targets", "?")}  '
              f'rank {_icfg.get("rank", "?")}  epoch {_ist.get("epoch", "?")}')
        print(f'[GATE-init] max|B| {_b4:.3e} -> {_af:.3e}')
        assert _af > 0, 'GATE-init FAILED: loaded B is still all zero'
        assert _icfg.get('mesh') != args.mesh, (
            'GATE-init FAILED: --init-from was trained on THIS mesh -- that is a '
            'resume, not a warm start from another object')
        print("[GATE-init] PASSED -- starting from another object's solution", flush=True)
