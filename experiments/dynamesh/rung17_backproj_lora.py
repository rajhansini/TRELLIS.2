...
ap.add_argument('--lambda-con', type=float, default=0.0,
                help="rung21. Weight on the EDIT-CONSISTENCY regulariser as a "
                     "FRACTION of the data loss (a ratio, not an absolute — that "
                     "is what makes it object-independent). Unsupervised voxels "
                     "are asked to receive the same edit the verified ones got. "
                     "NOT 'stay at frozen': frozen is globally dark (0.062 vs a "
                     "truth near 0.31), so anchoring to it yields a dark band, "
                     "measured at alpha=0.")
ap.add_argument('--lambda-smo', type=float, default=0.0,
                help='rung21. Same, for the neighbour-consistency regulariser.')
...
_CFG = dict(variant='trellis2_backproj_lora', loss_region='rendered_gt',
            rung=(21 if (args.lambda_con > 0 or args.lambda_smo > 0)
                  else 20 if args.conf_weight else 22 if args.lambda_smooth > 0
...
_RUNG = (21 if (args.lambda_con > 0 or args.lambda_smo > 0)
         else 20 if args.conf_weight
         else 22 if args.lambda_smooth > 0
         else {'kv': 17, 'kvo': 18, 'v': 19}.get(args.targets, 17))
LABEL  = f'rung{_RUNG}_{args.blocks}_{args.targets}_r{args.rank}_s{args.seed}_{RUN_ID}'
...
class CrossAttnLoRA(nn.Module):
    ...
    def __init__(self, ch, ctx, rank, targets=('to_q', 'to_kv', 'to_out'),
        ...
        if 'to_v' in targets:
            ...
            self.to_v = LoRALayer(ctx, ch, rank)
...
def main():
    ...
    if CONF_ON:
        ...
        _w = torch.where(renderer.cos_inc >= TAU, _w, torch.zeros_like(_w))
    ...
    for ep in range(start_ep, EPOCHS + 1):
        ...
        for si, fi in enumerate(order, 1):
            ...
            with lora_ctx(flow, reg):
                ...
                _m = inter_mask(fi)
                if args.conf_weight:
                    _se = ((img - gts[fi]) ** 2)[0][_m]          # [N,3]
                    _wm = conf_w[_m]                             # [N]
                    loss_data = (_wm[:, None] * _se).sum() / (_wm.sum() * 3).clamp_min(1e-9)
                else:
                    loss_data = ((img - gts[fi]) ** 2)[:, _m].mean()
                loss = loss_data
                if REG_ON:
