# =============================================================================
# rung26_mcfm_lora.py — rung20_conf_lora.py + MCFM temporal token blending.
#
# SEPARATE FILE BY DESIGN. rung20_conf_lora.py is NOT modified — it remains the
# exact script that produced the published baselines (rung17/18/19/25), verified
# byte-identical to its pre-MCFM backup. This is a copy with FOUR additions, all
# gated behind --mcfm:
#   1. --mcfm {v2_C,v2_D,v3_C,v3_D}
#   2. mcfm added CONDITIONALLY to the run hash, so the four modes cannot land in
#      one directory and resume from each other's checkpoints (the failure the
#      mesh/gt_dir comment below records as having cost two full runs)
#   3. mcfm added to the run LABEL, so the directory names the mode
#   4. one call to mcfm_blend.blend_conds() after the cond cache is built
#
# WHAT MCFM DOES: the flow's cross-attention conditioning is a cached dict
# {frame -> DINOv3 tokens}, built once before training. MCFM replaces each frame's
# tokens with a blend over a short temporal window, so the flow never sees vanilla
# per-frame tokens. Parameter-free; nothing in the blend is trained.
#
# Validated in TRELLIS 1 first (experiments/enhancement/step06b_asset.py, 8 runs on
# spot_lava and teapot_lava2). Tokens do not transfer — TRELLIS 1 is DINOv2
# (1374,1024), TRELLIS.2 is DINOv3 (1029,1024) — but the operation does, having no
# parameters.
#
# With --mcfm unset this file is equivalent to rung20_conf_lora.py: the hash entry
# and label suffix are absent and the blend call is skipped.
# =============================================================================
"""
...
ap.add_argument('--mcfm', default=None,
                ...
                help="MCFM temporal token blending applied to the cached DINOv3 "
                     "conditioning BEFORE training. v2 blends token i across the "
                     "window; v3 pools over time AND space. C=[t,t+1], "
                     "D=[t-1,t,t+1]. Parameter-free. Unset = vanilla, and then "
                     "this script is equivalent to rung20_conf_lora.py.")
args = ap.parse_args()
...
_CFG = dict(variant='trellis2_backproj_lora', loss_region='rendered_gt',
            rung=(24 if (args.conf_ramp_qlo >= 0 and args.conf_ramp_qhi >= 0)
            ...
            **({} if not args.lpips else dict(lpips=True)),
            # MCFM MODE. Conditional, like recon and lpips: absent reproduces the
            # baseline hashes exactly. Present is REQUIRED — the four modes differ
            # only here, so without it all four collide into one directory.
...
_RUNG = (24 if (args.conf_ramp_qlo >= 0 and args.conf_ramp_qhi >= 0)
         else 22 if (args.lambda_con > 0 and args.lambda_smo <= 0)
         else 21 if (args.lambda_con > 0 or args.lambda_smo > 0)
...
def main():
    ...
    print('=' * 92)
    print(f'RUNG {_RUNG} — LoRA on TRELLIS.2 texture-flow CROSS-ATTENTION')
    print(f'  run       : {LABEL}')
    ...
    # MCFM: replace the vanilla per-frame tokens with temporally blended ones.
    # Every downstream use of `conds` reads this dict, so one call covers them all.
    if args.mcfm:
