...
ap.add_argument('--k-bias', type=float, default=1.0,
                help="Shape of the training-timestep draw. k is a knot of the REAL "
                     "12-step schedule; the step pays k no-grad evals to reach it "
                     "plus 1 graded eval. Default 1.0 = uniform, exactly as before "
                     "(E[k]=5.5, 6.5 evals/step). p>1 biases k LOW: k=int(STEPS*u**p), "
                     "so E[k]=STEPS/(p+1). p=2 -> 5.0 evals (1.30x), p=3 -> 4.0 (1.63x). "
                     "TRADE-OFF, state it: T_SEQ runs 1->0, so LOW k is the HIGH-NOISE "
                     "end. Biasing k down therefore trains the adapter less often at "
                     "the low-noise timesteps that carry fine detail, which is exactly "
                     "what this file's design note says it wants to avoid.")
args = ap.parse_args()
...
import hashlib
_CFG = dict(k_bias=float(args.k_bias),
            variant='trellis2_backproj_lora', loss_region='rendered_gt',
...
_KBT = '' if abs(args.k_bias - 1.0) < 1e-9 else f'_kb{args.k_bias:g}'
LABEL  = f'rung{_RUNG}{_RTAG}{_KBT}_{args.blocks}_{args.targets}_r{args.rank}_s{args.seed}_{RUN_ID}'
...
def main():
    ...
    for ep in range(start_ep, EPOCHS + 1):
        ...
        for si, fi in enumerate(order, 1):
            ...
            if abs(args.k_bias - 1.0) < 1e-9:
                k = int(rng.integers(0, STEPS))      # uniform, unchanged
            else:
                k = min(STEPS - 1, int(STEPS * rng.random() ** args.k_bias))
            # a knot of the REAL schedule
            t_k = T_SEQ[k]
