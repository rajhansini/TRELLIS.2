"""render_rung37_orbit.py -- orbit/fixed-view renders for rung37 checkpoints.

WHY THIS FILE EXISTS. texel_after_train.sbatch called render_rung27_orbit.py for
every arm. A rung37 checkpoint carries three module families rung27's registry has
never heard of -- blocks_t.* (the second cross-attention branch), gates.* (one per
block) and tmix.* (the per-position temporal mixer, which IS rung37) -- so
load_state_dict died with "Unexpected key(s)" before a single frame rendered.
render_rung31_orbit.py does not rescue it either: it imports rung31, whose registry
has no tmix.

WHAT CHANGED FROM render_rung31_orbit.py. Three substitutions and nothing else:
the fake argv name, the trainer import, and the [R31] log tag. Everything else is
byte-identical, deliberately.

WHY THAT IS SUFFICIENT, despite rung31 and rung37 having different temporal
geometry. The renderer only ever (a) constructs R.LoRARegistry and (b) wraps each
block in R.DualCrossAttn(inner, n_img, reg, idx). Both signatures are identical
across the two trainers, and rung37's LoRARegistry builds tmix itself whenever
dual=True. The window-stacking block is also unchanged on purpose: rung37's
DualCrossAttn.forward still receives the stacked W*n_img context and derives W
from it, collapsing per position INSIDE the wrapper. So the caller-side contract
is the same even though the operator is not.
"""
...
sys.argv = ['rung37_perpos_temporal_lora.py',
            '--mesh', MESH,
            '--gt-dir', GT_DIR,
            ...
            [] if not CFG.get('temporal_window') else
            ['--temporal-window', str(CFG['temporal_window']),
             '--branch', CFG.get('branch', 'both'),
             '--gate-init', str(CFG.get('gate_init', 0.0))])
...
import rung37_perpos_temporal_lora as R          # noqa: E402
...
def main():
    ...
    reg = R.LoRARegistry(len(flow.blocks), flow.model_channels, flow.cond_channels,
                         ...
                         active=CFG.get('active'), alpha=_alpha,
                         dual=bool(CFG.get('temporal_window')),
                         branch=CFG.get('branch', 'both'),
                         gate_init=float(CFG.get('gate_init', 0.0))).to(DEVICE)
    ...
    # ── rung37: the renderer must reproduce TRAINING exactly ────────────────
    # Two things are required and neither is optional. Without the window the
    # adapter sees 1029 tokens where it was trained on 3087; without the wrapper
    # the second cross-attention pass never runs and the temporal LoRA is dead
    # weight. Either omission renders a DIFFERENT model and the video would not
    # correspond to the reported PSNR.
    _TW = int(CFG.get('temporal_window') or 0)
    if _TW:
        # BOUND BY WHAT IS ACTUALLY LOADED. _raws holds only the frames this
        # sweep needs; the fixed-view path loads fewer than CFG['n_frames'], and
        # indexing past the end is what made 24 of the first 28 render jobs die
        # with IndexError in cond_image.
        _n = min(ARGS.n_frames if ARGS.sweep != 'angle' else CFG['n_frames'], len(_raws))
        if _CONDS is None:
            with torch.no_grad():
                _CONDS = {i: pipe.get_cond([cond_image(i - 1)], CFG['resolution'])['cond']
                          for i in range(1, _n + 1)}
        _k = sorted(_CONDS); _lo, _hi = _k[0], _k[-1]
        _half = _TW // 2
        _off = list(range(-_half, _half + 1))
        _nimg = int(next(iter(_CONDS.values())).shape[-2])
        _b = dict(_CONDS)
        _CONDS = {f: torch.cat([_b[min(max(f + o, _lo), _hi)] for o in _off],
                               dim=(1 if _b[f].dim() == 3 else 0)) for f in _k}
        _mf = _k[len(_k) // 2]
        assert torch.equal(_CONDS[_mf][..., _half * _nimg:(_half + 1) * _nimg, :], _b[_mf]), \
            'GATE-window FAILED in renderer: middle slice is not frame f'
        for _i in (CFG.get('active') or range(len(flow.blocks))):
            flow.blocks[_i].cross_attn = R.DualCrossAttn(
                flow.blocks[_i].cross_attn, _nimg, reg, _i)
        log(f'[R37] window={_TW} tokens {_nimg}->{_nimg*_TW}  '
            f'GATE-window PASSED  DualCrossAttn on {len(list(CFG.get("active") or range(len(flow.blocks))))} blocks')
