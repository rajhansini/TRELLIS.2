...
ap.add_argument('--tag', default='rung14_t2_orbit')
...
def main():
    ...
    log('=' * 88)
    log('RUNG14 (TRELLIS.2) TURNTABLE — frozen | cross-attn LoRA')
    ...
    if ARGS.sweep == 'angle':
        ...
        n = ARGS.n_frames
        items = [(i, i + 1, i * 360.0 / n) for i in range(n)]
    yaws = items
    for k, (ci, fr, yaw) in enumerate(items, 1):
        ...
        panels = ([(a_fz, 'TRELLIS.2 (pure, no adapter)')] if ARGS.frozen_only
                  else [(a_fz, 'TRELLIS.2 frozen'), (a_lo, 'rung14 cross-attn LoRA')])
    ...
    vid = OUT / f'RUNG14_T2_{ARGS.sweep.upper()}_frozen_vs_xattn.mp4'
